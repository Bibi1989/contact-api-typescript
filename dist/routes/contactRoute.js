...
router.put('/:updateID', (req, res) => {
    try {
        ...
        contacts_1.default.findByIdAndUpdate(updateID, updateContact, { new: true }).then(doc => res.status(200).json({ message: "Contact Updated", data: doc })).catch(err => res.status(404).json({ msg: err }));
    }
    catch (error) {
        res.status(404).json({
            msg: "Invalid Id"
